...
playerService.move = (query, cb) => {
  ...
  mapService.get({ targetMapId: result[0].playerMap }, (err, map) => {
    ...
    _.forEach(map.mapNext, (next) => {
      if (next === query.newPlayerMap)
        isNext = true;
    });

    if (result[0].playerMap === query.newPlayerMap || isNext) {
      result[0].playerMap = query.newPlayerMap;
...
const filterHandlers = {
  ...
  targetMapId: (list, id) => {
    return list.filter((doc) => {
      return !!~doc.playerMap.indexOf(id);
    });
  },
...
playerService.challengeOne = (query, cb) => {
  var list = playerCache.list;
  var result = filterHandlers.targetPlayerId(list, "Use39865c41498e0dde29c8403b859ab53");
  //var player = result[0];
  //
...
playerService.challengeTwo = (query, cb) => {
  var list = playerCache.list;
  var result = filterHandlers.targetPlayerId(list, "Usa2547b86f809a8385e1901dda2cb4597");
  //var player = result[0];
  //
  //var chalengers = getChallenger(player);
  //
...
playerService.challengeFour = (query, cb) => {
  ...
  if (itemOne.itemValue > itemTwo.itemValue) {
    ...
    playerService.updatePlayerExp({ targetPlayerId: playerOne.playerId, calcValue: paidExp } , () => {
      playerService.updatePlayerExp({ targetPlayerId: playerTwo.playerId, calcValue: -paidExp } , () => {
        playerOne.playerItems.push(itemTwo.itemId);
        _.remove(playerTwo.playerItems, (n) => {
          return n === itemTwo.itemId;
        });
  ...
  if (itemTwo.itemValue > itemOne.itemValue) {
    ...
    playerService.updatePlayerExp({ targetPlayerId: playerTwo.playerId, calcValue: paidExp } , () => {
      playerService.updatePlayerExp({ targetPlayerId: playerOne.playerId, calcValue: -paidExp } , () => {
        playerTwo.playerItems.push(itemOne.itemId);
        _.remove(playerOne.playerItems, (n) => {
          return n === itemOne.itemId;
        });
        cb(null, [playerOne, playerTwo]);
      });
    });
...
playerService.challengeFive = (query, cb) => {
  var list = playerCache.list;
  ...
  _.forEach(result, (player, index) => {
    var targetPlayer;
    if (index === 0) {
      targetPlayer = result[result.length-1];
    } else {
      targetPlayer = result[index-1];
    }

    var damage = 0;
    if (player.playerAtk > player.playerInt) {
      damage = getMax(1, player.playerAtk - targetPlayer.playerDef) * getMax(1, player.playerAgi - targetPlayer.playerAgi);
    } else if (player.playerAtk < player.playerInt) {
      damage = getMax(1, player.playerInt - targetPlayer.playerInt) * _.floor(player.playerMp * 0.1);
    }

    var tmpHp = targetPlayer.playerHp;
    targetPlayer.playerHp -= damage;

    if (targetPlayer.playerHp <= 0) {
      if (!_.isEmpty(targetPlayer.playerItems))
          targetPlayer.playerItems.unshift();
      targetPlayer.playerHp = tmpHp;
    }
  });
