...
playerService.challenge1 = (query, cb) => {
  var list = playerCache.list;
  var result = filterHandlers.targetPlayerId(list, query.targetPlayerId);
  var player = result[0];
  ...
  var targetPlayers = _.filter(list, (_player) => {
    return player.playerMap === _player.playerMap;
  });
...
function getStrong(player) {
  return (player.Hp + player.Mp) * (player.Atk * player.Hp + player.int * player.Mp) * player.Def * player.Agi;
}
